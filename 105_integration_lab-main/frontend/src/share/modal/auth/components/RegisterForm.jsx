...
const RegisterForm = ({ setIsLogin = () => {}, setStatus = () => {} }) => {
  ...
  const handleSubmit = async()=>{
    ...
    const response = awaitAxios.post('/register',{username,email,password,}
    );
    ...
    setPassword('');
    setRePassword('');
    // check if e are AxiosErrorif(einstanceofAxiosError)
    if(e.response)
    // check if e.response exist
    return setStatus({
      msg:e.response.data.error,severity:'error',});
...
const validateForm = ()=>{
  ...
  // check user
  if (!username) {
    setUsernameError('Username is required');isValid=false;
  }
    // check email
    if(!email) {
      setEmailError('Email is required');isValid=false;
    }if(!/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g.test(email)) {
      setEmailError('Invalid email format');isValid=false;
    }
      // check password
      if(!password) {
